...
mod_av_set(VALUE klass, ID id, VALUE val, int isconst)
{
	...
	if (st_lookup(RCLASS_IV_TBL(klass), (st_data_t)id, &value)) {
		...
		rb_funcall(value, rb_intern("undef_class"), 0, 0);
                /*
                st_delete(rb_class_tbl, id, 0); // st_add_direct(rb_class_tbl, id, klass);
                rb_ivar_set(klass, value, 0); // rb_name_class(klass, id);
                rb_const_remove(klass, value); // rb_const_set(rb_cObject, id, klass);
                */
	    }
	    else
